...
void sr_process_generic_response_indication(pkt_buf_t * pkt)
{
	...
	//printf("sr_process_generic_response_indication: Received GW ZIGBEE_GENERIC_RSP_IND.\r\n");

	msg = gw_zigbee_generic_rsp_ind__unpack(NULL, pkt->header.len, 
	pkt->packed_protobuf_packet); 

	//printf("sr_process_generic_response_indication: Received GW ZIGBEE_GENERIC_RSP_IND, %d\r\n",  msg->sequencenumber);
	if (msg)
	{
...
#ifdef ASE
						/* Inform on/off request status to application. */
						InformEventToApp(CB_FLAG_ON_OFF_STATUS, msg, &pending_attribs[i]); 
#endif
		...
		{
			//Search for the sequence number in the table, and when found delete the entry
			//printf("1sr_process_generic_response_indication: searching for seq_num=%d\r\n", msg->sequencenumber);	
	
#ifdef ASE	
			/* Inform on/off request status to application. */
			//InformEventToApp(CB_FLAG_ON_OFF_STATUS, msg, &pending_attribs[0]); 
#endif
