...
int si_send_packet(pkt_buf_t * pkt, confirmation_processing_cb_t _confirmation_processing_cb, void * _confirmation_processing_arg)
{
	...
	{
//		ui_print_status(0, 
		//printf("MAVENDEBUG, Please wait while connecting to server\r\n");
		return -1;
	}
	...
	{
		//ui_print_status(0, 
		//printf("MAVENDEBUG, BUSY - please wait for previous operation to complete\r\n");
		return -1;
	}
	...
	{
//		UI_PRINT_LOG
		//print("MAVENDEBUG,sending packet to %s\n", server->name);
		return -1;
	}
...
//	UI_PRINT_LOG("MAVENDEBUG, Init state %d\r\n", state);
...
void si_deinit(void)
{
	tcp_disconnect_from_server(&gateway_server);
	tcp_disconnect_from_server(&network_manager_server);
	tcp_disconnect_from_server(&ota_server);
	//TBD
}
