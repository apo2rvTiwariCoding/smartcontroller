...
#define HIDE_CURSOR			"HC"
#define SHOW_CURSOR			"SC"

#define SCROLLING_ALL		"SA"
#define SCROLLING_(FROM,TO)	"SCR"
#define SCROLLING(FROM,TO)	printf("%d;%d", (FROM), (TO))

#define SCROLL_DOWN			"SD"
#define SCROLL_UP			"SU"

#define LOCATE_(Y,X) "LO"
#define LOCATE(Y,X) 
//#define LOCATE(Y,X) printf("%d;%d",(Y),(X))
#else
...
enum
{
	...
	ACTION_SCENE_RECALL,
	ACTION_SEND_ASE_GEN_REQ_PKT,	/* ASE change: Action to be perform to send generic ASE request packet to ASE device.  */
	ACTION_SEND_ASE_GEN_RESP_PKT,	/* ASE change: Action to be perform to send generic ASE response packet to ASE device. */
	ACTION_QUIT
};
...
char * ACTION_NAMES[] =
{
	...
	"SCENE_RE&CALL",
	"SEND_&IPKT", 	/* ASE change: Action to be displayed on user interafce to send ASE request packet. When character 'I' is pressed generic request packet will be send to ASE device. */
	"SEND_&JPKT",   /* ASE change: Action to be displayed on user interafce to send ASE response packet. When character 'J' is pressed generic response packet will be send to ASE device. */ 
	"&QUIT"
};
...
char ** ACTION_SUBNAMES[] = 
{
	...
	ACTION_SUBNAMES_8,
	NULL,
	NULL,	/* ASE change: No sub action to be displayed on user interface for generic request packet.*/
	NULL,	/* ASE change: No sub action to be displayed on user interface for generic response packet.*/
	NULL
};
...
uint16_t action_value[NUM_OF_ACTIONS] =
{
	...
	0,
	0,
	100,	/* ASE change: Unqiue number to identify generic request action. */
	101,	/* ASE change: Unqiue number to identify generic response action. */
	0
};
...
uint16_t ACTION_VALUE_MAX[NUM_OF_ACTIONS] =
{
	...
	2,
	2,
	1,	/* ASE change: Number of action that is possible to take for generic request action. */
	1,	/* ASE change: Number of action that is possible to take for generic response action. */
	1,	
	1
};
...
uint16_t ACTION_TYPE[NUM_OF_ACTIONS] =
{
	...
	ACTION_TYPE_STRING,
	ACTION_TYPE_STRING,
	ACTION_TYPE_TRIGGER,	/* ASE change: Type of user action for generic request. Different action like string comparision or trigger. For request user action is trigger. */
	ACTION_TYPE_TRIGGER,	/* ASE change: Type of user action for generic response. Different action like string comparision or trigger. For response user action is trigger. */
	ACTION_TYPE_TRIGGER,	
	ACTION_TYPE_TRIGGER
};
...
#ifndef ASE
/* ASE change: This function is use to fill the command packet that to send to ASE device through custom cluster i.e. FC01.
...
*/
unsigned int FillDefaultCommand(char *CmdBuffer, unsigned char IsReqFlag)
{
	time_t Time;
	struct tm *Tm;
		...
		if (escape_state == 0)
		{
			switch (c)
			{
				...
				case 'P':
					...
					perform_action = true;
					break;
				/* ASE change: To send request packet to ASE device through custom cluster when character 'i'/'I' is pressed. */
				case 'i':
				case 'I':
					current_action = ACTION_SEND_ASE_GEN_REQ_PKT;
					perform_action = true;
					break;
				/* ASE change: To send response packet to ASE device through custom cluster when character 'j'/'J' is pressed. */
				case 'j':
				case 'J':
					current_action = ACTION_SEND_ASE_GEN_RESP_PKT;
					perform_action = true;
					break;
				case 10: //[ENTER]
					perform_action = true;
			...
			if (perform_action)
			{
				...
				if ((current_action == ACTION_ON_OFF) 		
					|| (current_action == ACTION_SEND_ASE_GEN_REQ_PKT)	/* ASE change: When charcater is pressed on UI, identify the action to be perform and take the action. For sending the request packet through the custom cluster added action which will send request packet to ASE device. */
					|| (current_action == ACTION_SEND_ASE_GEN_RESP_PKT)	/* ASE change: When charcater is pressed on UI, identify the action to be perform and take the action. For sending the response packet through the custom cluster added action which will send response packet to ASE device. */
					|| (current_action == ACTION_LVL)
					|| (current_action == ACTION_HUE)
					|| (current_action == ACTION_SAT)
					|| (current_action == ACTION_GROUP_ADD_REMOVE)
					...
					{						
						switch (current_action)
						{
							case ACTION_ON_OFF:
								...
								break;
							/* ASE change: Invoking the fill function which will send the request packet to ASE device through custom cluster. */
							case ACTION_SEND_ASE_GEN_REQ_PKT:

								{
									//unsigned int Length = 0;
									//char CmdBuffer[120] = {0};
									uint32_t rfChannel = 0x07FFF800;
									uint64_t panId = 0x0047;
									unsigned char secKey[16] = {0};

									/* Filling default command to send request packet to ASE deviec througth custom cluster.*/
									//Length = FillDefaultCommand(CmdBuffer, 1);

									/* Sending the packet */
									//act_send_ase_gen_req_pkt(&destination_addr, Length, CmdBuffer);
			                        			comm_send_nwk_start(&rfChannel, &panId, 1, secKey, 0);

									break;
								}
							/* ASE change: Invoking the fill function which will send the response packet to ASE device through custom cluster. */
							case ACTION_SEND_ASE_GEN_RESP_PKT:
								{
									//unsigned int Length = 0;
									//char CmdBuffer[120] = {0};

									/* Filling default command to send response packet to ASE device through custom cluster. */
									//Length = FillDefaultCommand(CmdBuffer, 0);

									/* Sending the packet */
									//act_send_ase_gen_resp_pkt(&destination_addr, Length, CmdBuffer);
