...
#ifdef ASE
/* ASE Change */
/*! brief API used as call back when network is formed with specified pan id and channel number.
 
	\param pkt It contains message on successful formation of network.
	\param cbArg It stores extra information but not used now.

	\return Returns NOTHING.
*/ 
void comm_process_nwk_start (pkt_buf_t *pkt, void *cbArg) 
{
	...
	printf("comm_process_nwk start: Received ZIGBEE_GENERIC_CNF\r\n");
	...
	if (msg)
	{
		if (msg->status == NWK_STATUS_T__STATUS_SUCCESS)
		{
			printf("comm_process_nwk start: Status SUCCESS.");	
		}
		else
		{
			printf("comm_process_nwk_start: Error: Status FAILURE.");
		}
	...
	{
		printf("comm_process_nwk_start: Could not unpack msg");
	}
}
#endif
...
#ifdef ASE
/* ASE Change */
/*! brief API used to send request to form network. It will form the network with parameters specified e.g. pan id and channel number on which network is to be formed.
	\param rfChannel Specifies channel number on which network is to be formed. Valid channel number for ZigBee is 11 to 26.
	\param panId Specifies unique id to form network.
	\param deviceType Specifies Type of device e.g. coordinator, router and end device.

	\return Returns 0 on successful sending request to network manager. On failure returns non zero value.
*/ 
signed char comm_send_nwk_start(uint32_t *rfChannel, uint32_t *panId, uint8_t deviceType)
{
	pkt_buf_t * pkt = NULL;
	uint8_t len = 0;
	NwkStartReq msg = NWK_START_REQ__INIT; 

	msg.nwkchannel = *rfChannel; 
  	msg.panid = *panId;	
	msg.type = deviceType;
	len = nwk_start_req__get_packed_size(&msg);
	pkt = malloc(sizeof(pkt_buf_hdr_t) + len);

	if (pkt)
	{
		printf("comm_satrt_nwk: Send request to network manager..\r\n");
		pkt->header.len = len;
		pkt->header.subsystem = Z_STACK_NWK_MGR_SYS_ID_T__RPC_SYS_PB_NWK_MGR;
		pkt->header.cmd_id= NWK_MGR_CMD_ID_T__NWK_START_REQ;

		nwk_start_req__pack(&msg, pkt->packed_protobuf_packet);

		if (si_send_packet(pkt, &comm_process_nwk_start, NULL) !=0 )
		{
			printf("comm_satrt_nwk: Error: Could not send msg..\r\n");
	...
	{
		printf("comm_start_nwk: Error: Could not unpack msg..\r\n");
...
signed char comm_send_permit_join(uint8_t joinTime)
{
	pkt_buf_t * pkt = NULL;
	uint8_t len = 0;
	...
	//printf("comm_send_permit_join: Sending NWK_SET_PERMIT_JOIN_REQ with Join Time 0x%x\r\n", joinTime);
	...
	if (pkt)
	{
		...
		{
			printf("comm_send_permit_join,  Could not send msg\r\n");
	...
	{
		printf("comm_send_permit_join,  Could not unpack msg\r\n");
...
signed char comm_remove_device_request(zb_addr_t * addr)
{
	pkt_buf_t * pkt = NULL;
	uint8_t len = 0;
	...
	if (pkt)
	{
		...
		{
			printf("comm_remove_device_request, Could not send msg..\r\n");
	...
	{
		printf("comm_remove_device_request, Could not pack msg\r\n");
