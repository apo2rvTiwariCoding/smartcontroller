...
void snsr_process_power_response(pkt_buf_t * pkt)
{
	...
	//		printf("snsr_process_power_response: SUCCESS (%d)\r\n", msg->status);
#ifdef ASE
			/* Inform the power consumption of smart meter to application. */
			InformEventToApp(CB_FLAG_READ_SMART_METER_STATUS, msg, NULL);
#endif
			ui_refresh_display();
		}
		else
	...
	//		printf("snsr_process_power_response: FAILURE (%d)\r\n", msg->status);
		}
...
signed char snsr_get_power(zb_addr_t addr)
{
	pkt_buf_t * pkt = NULL;
	uint8_t len = 0;
	...
	if (pkt)
	{
		...
		if (si_send_packet(pkt,(confirmation_processing_cb_t)&snsr_process_confirmation, NULL) != 0)
		{
			...
			//printf("snsr_get_power: Error: Could not send msg.\r\n");
	...
	else
	{
		...
		//printf("snsr_get_power: Error: Could not pack msg.\r\n");
