...
static void processNwkStartReq(NwkStartReq *pStartReq ); 	/* ASE Change: Send network start request to zdo. It forms network with parameters specified in network configuration like pna id, channel list got from application layer.*/
static void processSetNwkParam(NwkStartReq *pNativeMsg); 	/* ASE Change: Set the network start paramter got from application layer. Parameters e.g. pan id and channel list on which metwork is to form. Write parameters on NV.*/
                      ...
                      uint8 *pData, uint8 type )
{
  ...
  switch ( cmdId )
  {
    ...
    case NWK_MGR_CMD_ID_T__NWK_SET_PERMIT_JOIN_REQ:
      ...
      /* ASE Change: */
      /* Request of network start to network manager. Network manager send request on ZDO layer to start network. */
     case NWK_MGR_CMD_ID_T__NWK_START_REQ:
      pNativeMsg = nwk_start_req__unpack( NULL, len, pData );
      if ( pNativeMsg )
      {
	/* Set the network start paramter got from application layer. Parameters e.g. Pan id and channel list on which metwork is to form. */
	processSetNwkParam((NwkStartReq *)pNativeMsg );
	uiPrintf("ASE PKT START AT NWK MNGR..\r\n");

	/* Send request of network start on ZDO layer to form network. */
        processNwkStartReq((NwkStartReq *)pNativeMsg );
        nwk_start_req__free_unpacked( pNativeMsg, NULL ); 
      }
      break;
...
static void processNwkZigbeeNwkInfoReq( int connection, NwkZigbeeNwkInfoReq *pNwkInfoReq )
{
  ...
  //printf("MAVENDEBUG, ,!!!!! 1. STATUS, %d !!!!\r\n", nwkInfoCnf.status);
  // synchronous call, gets info into gLocalDeviceInfo
  sendSysNwkInfoReadReq();
  //printf("MAVENDEBUG, ,!!!!! 2.STATUS, %d DEV STATE, %d, !!!!\r\n", nwkInfoCnf.status, gLocalDeviceInfo.devstate);
  
  if ( ( gLocalDeviceInfo.devstate == DEV_STATE__DEV_ROUTER ) || (gLocalDeviceInfo.devstate == DEV_STATE__DEV_ZB_COORD ) )
  {
	 // printf("MAVENDEBUG, !!!! NWK STATE UP...!!!!\r\n");
    nwkInfoCnf.status = NWK_NETWORK_STATUS_T__NWK_UP;
  }
  else
  {
	//printf("MAVENDEBUG, !!!! NWK STATE DOWN...!!!!\r\n");
...
/**************************************************************************************************
 *
 * @fn          sendZdoMgmStartNwkReq
 *
 * @brief       Send ZDO network start request with network configuration parameters.
 *
 * @param       pNwkStartReq - Network configuration parameters e.g. pan id and channel list on which network is to formed. 
 *
 * @return      none
*/
static void sendZdoMgmStartNwkReq( DevStartReq *pNwkStartReq )
{
  int len;
  uint8 *pBuf;
   
  len = dev_start_req__get_packed_size( pNwkStartReq );
  pBuf = malloc( len );
  if ( pBuf )
  {
    len = dev_start_req__pack( pNwkStartReq, pBuf );

    uiPrintf("ASE START PKT.. SEND TO GTWAY SRVER..\r\n");
    sendAPICExpectDefaultStatus( ZSTACK_CMD_IDS__DEV_START_REQ, len, pBuf );
...
/* ASE Change: */
/*********************************************************************
 * @fn      sendZStackSetNwkParamReq
 *
 * @brief   It send request to zstack to store network configuration parameter in NV.
 *
 * @param   pNwkSetParam - Network configuartion information to store in NV e.g. pan id and channel list.
 *
 * @return  none
 */
static void sendZStackSetNwkParamReq(SysConfigWriteReq *pNwkSetParam)
{
  int len;
  uint8 *pBuf;
  ZStatusValues status = ZSTATUS_VALUES__ZMemError;

  len = sys_config_write_req__get_packed_size(pNwkSetParam);
  pBuf = malloc( len );
  if ( pBuf )
  {
	len = sys_config_write_req__pack( pNwkSetParam, pBuf );

	status = sendAPICExpectDefaultStatus( ZSTACK_CMD_IDS__SYS_CONFIG_WRITE_REQ, len, pBuf );

	printf("Nwk set param, Status, %d\r\n", status);
...
/* ASE Change: */
/*********************************************************************
 * @fn      processSetNwkParam
 *
 * @brief   Set the network start paramter got from application layer. Parameters e.g. pan id and channel list on which metwork is to form. Write parameters 		 on NV.
 *
 * @param   pNativeMsg - Stores information required to form network e.g. channel list and pan id.
 *
 * @return  none
 */
static void processSetNwkParam(NwkStartReq *pNativeMsg)
{
  SysConfigWriteReq SetNwkParam = SYS_CONFIG_WRITE_REQ__INIT;

  /* For Storing channel list and pan id in NV set flags for them and store paramters got from application layer. */
  SetNwkParam.has_chanlist = 1;
  SetNwkParam.chanlist = pNativeMsg->nwkchannel;

 if(pNativeMsg->panid != 0xFFFF && pNativeMsg->panid != 0x0000) 
 {
  	SetNwkParam.has_panid = 1;
  	SetNwkParam.panid = pNativeMsg->panid;
 }

 /* Send request to store network configuration parameter to NV. */
 sendZStackSetNwkParamReq(&SetNwkParam);

}

/* ASE change: */
/*********************************************************************
 * @fn      processNwkStartReq
 *
 * @brief   Send network start request to zdo. It forms network with parameters specified in network configuration like pna id, channel list got from appl		 ication layer.
 *
 * @param   pStartReq - Stores network related information like pan id, channel list.
 *
 * @return  none
 */
static void processNwkStartReq(NwkStartReq *pStartReq )
{
  /* Initialize the parameters. */
  DevStartReq mgmtNwkStartReq = DEV_START_REQ__INIT;

  mgmtNwkStartReq.startdelay = 0;

  /* Send request on zstack to start network. */
  sendZdoMgmStartNwkReq(&mgmtNwkStartReq);
...
ZStatusValues sendSysNwkInfoReadReq( void )
{
  ...
  if ( pBuf )
  {
    ...
    //printf("MAVENDEBUG, ,!!!!! BEFOR, Global dev state, %d !!!!!\r\n", gLocalDeviceInfo.devstate);
    // Send protobuf packed request to API Client synchronously
    pRsp = apicSendSynchData( NM_API_CLIENT, ZSTACK_SYS_IDS__RPC_SYS_PROTOBUF, 
