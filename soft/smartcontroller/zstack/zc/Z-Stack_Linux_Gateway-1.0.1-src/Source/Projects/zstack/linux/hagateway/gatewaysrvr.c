...
static ZStatus_t sendDevASEGenReq( DevSendASEGenReq *pASEGenReq ); /* ASE change: To send request packet on ZigBee fill packet with neccessary information like PAN Id, short address, extended address, end point. */
static ZStatus_t sendDevASEGenResp( DevSendASEGenResp *pASEGenResp ); /* ASE change: Perapare response packet that to be sent to ASE device. Required details from  incoming App protobuf message is paresed and prepare packet that will contains information required to send it on ZigBee e.g. ieee address. */
...
static void processDevSendASEGenReq( int connection, DevSendASEGenReq *pASEGenReq );	/* ASE change: Perapare request packet that to be send on ZieBee and give confirmation to upper layer. */
static void processDevSendASEGenResp( int connection, DevSendASEGenResp *pASEGenResp );	/* ASE change: Perapare response packet that to be send on ZieBee and give confirmation to upper layer. */
...
static ZStatus_t zclGeneral_ProcessASECustom( zclIncoming_t *pInMsg );
...
static ZStatus_t zcl_HdlIncoming( zclIncoming_t *pInMsg )
{
  ...
  if ( zcl_ClusterCmd( pInMsg->hdr.fc.type ) )
  {
    ...
    if ( pInMsg->hdr.fc.manuSpecific == 0 )
    {
      ...
      else if( (pInMsg->msg->clusterId == ZCL_CLUSTER_ID_HA_ASE_CUSTOM) )
      {
	      status = zclGeneral_ProcessASECustom( pInMsg ); 
      }
    }
  }
...
void gwHandlePbCb( int connection, uint8 subSys, uint8 cmdId, uint16 len, uint8 *pData, uint8 type )
{
 ...
 printf(", %s, %d, %s, ANANDV, HERE \r\n", __func__, __LINE__, __FILE__);
  if ( (subSys & RPC_SUBSYSTEM_MASK) == Z_STACK_GW_SYS_ID_T__RPC_SYS_PB_GW )
  {
	...
	/* ASE change: Handle command id for sending the request packet to ASE device from upper layer. Received packet prepared according to protobuf so unapck it and prepeare the packet to send it on ZigBee. */
      case GW_CMD_ID_T__DEV_SEND_ASE_GEN_REQ:
        {
          pNativeMsg = dev_send_asegen_req__unpack( NULL, len, pData );
          if ( pNativeMsg )
          {
            processDevSendASEGenReq( connection, (DevSendASEGenReq *)pNativeMsg );
            dev_send_asegen_req__free_unpacked( pNativeMsg, NULL ); 
          }
        }
        break;

	/* ASE change: Handle command id for sending the response packet to ASE device from upper layer. Received packet prepared according to protobuf so unapck it and prepeare the packet to send it on ZigBee. */
      case GW_CMD_ID_T__DEV_SEND_ASE_GEN_RESP:
        {
          pNativeMsg = dev_send_asegen_resp__unpack( NULL, len, pData );
          if ( pNativeMsg )
          {
            processDevSendASEGenResp( connection, (DevSendASEGenResp *)pNativeMsg );
            dev_send_asegen_resp__free_unpacked( pNativeMsg, NULL ); 
          }
        }
        break;
...
static ZStatus_t zclGeneral_ProcessASECustom( zclIncoming_t *pInMsg )
{
	...
	/* Convert address from msg into gateway address struct */
	gwConvertAddrAfToPbRsp( &srcAddress, &(pInMsg->msg->srcAddr) );

	printf("%s, ASE CUSTOM CMD, DIR, %d, CMDID, %d\r\n", __func__, pInMsg->hdr.fc.direction, pInMsg->hdr.commandID);
	if ( zcl_ServerCmd( pInMsg->hdr.fc.direction ) )
	{
		switch ( pInMsg->hdr.commandID )
		{
			case COMMAND_ASE_CUSTOM_REQ:
				{
					DevSendASEGenReq reqASEGen;

					printf("%s, COMMAND_ASE_CUSTOM_REQ, SHRT ADDR, %x, L, %d\r\n", __func__, pInMsg->msg->srcAddr.addr.shortAddr, pInMsg->pDataLen);

					/* Initialize the structure */
					dev_send_asegen_req__init(&reqASEGen);

					/* Fill the structure */
					reqASEGen.apppkt.len = pInMsg->pDataLen;
					reqASEGen.apppkt.data = pInMsg->pData;
					reqASEGen.dstaddress = &srcAddress;

					/* Get buffer size to allocate */
					len = dev_send_asegen_req__get_packed_size(&reqASEGen);

					pBuf = malloc ( len );
					if(pBuf != NULL)
					{
						/* Fill packet */
						dev_send_asegen_req__pack(&reqASEGen, pBuf);

						/* Send response back to app */
						APIS_SendData( ALL_APP_CONNECTIONS, Z_STACK_GW_SYS_ID_T__RPC_SYS_PB_GW, TRUE, 
								GW_CMD_ID_T__DEV_SEND_ASE_GEN_REQ, len, pBuf );
			...
			case COMMAND_ASE_CUSTOM_RESP:
				{
					DevSendASEGenResp respASEGen;

					printf("%s, COMMAND_ASE_CUSTOM RESP, SHRT ADDR, %x, L, %d\r\n", __func__, pInMsg->msg->srcAddr.addr.shortAddr, pInMsg->pDataLen);

					/* Initialize the structure */
					dev_send_asegen_resp__init(&respASEGen);

					/* Fill the structure */
					respASEGen.apppkt.len = pInMsg->pDataLen;
					respASEGen.apppkt.data = pInMsg->pData;
					respASEGen.dstaddress = &srcAddress;

					/* Get buffer size to allocate */
					len = dev_send_asegen_resp__get_packed_size(&respASEGen);

					pBuf = malloc ( len );
					if(pBuf != NULL)
					{
						/* Fill packet */
						dev_send_asegen_resp__pack(&respASEGen, pBuf);

						/* Send response back to app */
						APIS_SendData( ALL_APP_CONNECTIONS, Z_STACK_GW_SYS_ID_T__RPC_SYS_PB_GW, TRUE, 
								GW_CMD_ID_T__DEV_SEND_ASE_GEN_RESP, len, pBuf );
...
/* ASE change: Perapare request packet that to be send on ZieBee and give confirmation to upper layer. */
/*********************************************************************
 * @fn      processDevSendASEGenReq
 *
 * @brief   Handles incoming App Protobuf messages.
 *
 * @param   connection - connection handle (tcp)
 * @param   pASEGenReq - pointer to received message structure
 *
 * @return  none
 */
static void processDevSendASEGenReq( int connection, DevSendASEGenReq *pASEGenReq )
{
  ZStatus_t status;
  
  uiPrintf( "Processing Send ASE Generic Request\n" );
  
  // send to ZigBee ASE devices
  status = sendDevASEGenReq( pASEGenReq );

  uiPrintf("###### Status, %d #####\r\n", status);
  
  // handle the response to the app
  gwHandleCnfRsp( GW_RSP_GENERIC, pASEGenReq->dstaddress->addresstype, 
                  connection, pASEGenReq->dstaddress->ieeeaddr, 
                  GW_CMD_ID_T__DEV_SET_ONOFF_STATE_REQ, status );

  // some debug code
  if ( status != ZSuccess )
  {
    uiPrintf( "Processing Send ASE Generic Request Failed - Status: %d\n", status );
  }
}

/* ASE change: Perapare response packet that to be send on ZieBee and give confirmation to upper layer. */
/*********************************************************************
 * @fn      processDevSendASEGenResp
 *
 * @brief   Handles incoming App Protobuf messages.
 *
 * @param   connection - connection handle (tcp)
 * @param   pASEGenResp - pointer to received message structure
 *
 * @return  none
 */
static void processDevSendASEGenResp( int connection, DevSendASEGenResp *pASEGenResp )
{
  ZStatus_t status;
  
  uiPrintf( "Processing Send ASE Generic Response\n" );
  
  // send to ZigBee ASE devices
  status = sendDevASEGenResp( pASEGenResp );
  
  // handle the response to the app
  gwHandleCnfRsp( GW_RSP_GENERIC, pASEGenResp->dstaddress->addresstype, 
                  connection, pASEGenResp->dstaddress->ieeeaddr, 
                  GW_CMD_ID_T__DEV_SEND_ASE_GEN_RESP, status );

  // some debug code
  if ( status != ZSuccess )
  {
    uiPrintf( "Processing Send ASE Generic Response Failed - Status: %d\n", status );
  }
}
...
/* ASE change: To send request packet on ZigBee fill packet with neccessary information like PAN Id, short address, extended address, end point. */
/**************************************************************************************************
 *
 * @fn          sendDevASEGenReq
 *
 * @brief       Send generic ASE request packet
 *
 * @param       pASEGenReq - command structure
 *
 * @return      ZStatus_t
 *
 **************************************************************************************************/
static ZStatus_t sendDevASEGenReq( DevSendASEGenReq *pASEGenReq )
{
  bool disableDefaultRsp;
  uint8 addrModeMask = ADDR_MODE_UNICAST + ADDR_MODE_GROUPCAST + ADDR_MODE_BROADCAST; // valid addr mode(s) 
  afAddrType_t dstAddr;
  
  uiPrintf( "Sending ASE Generic Request\n" );
  
  if ( gwConvertAddrPbToAfReq( pASEGenReq->dstaddress, &dstAddr, &disableDefaultRsp, addrModeMask ) )
  {
	  disableDefaultRsp = TRUE;
	 return zcl_SendCommand(GW_EP, &dstAddr, ZCL_CLUSTER_ID_HA_ASE_CUSTOM, COMMAND_ASE_CUSTOM_REQ, TRUE, ZCL_FRAME_CLIENT_SERVER_DIR, disableDefaultRsp, 0, zcl_TransID, pASEGenReq->apppkt.len, pASEGenReq->apppkt.data);
  }

  return ZInvalidParameter;
}

/* ASE change: To send response packet on ZigBee fill packet with neccessary information like PAN Id, short address, extended address, end point. */
/**************************************************************************************************
 *
 * @fn          sendDevASEGenResp
 *
 * @brief       Send generic ASE response packet
 *
 * @param       pASEGenResp - command structure
 *
 * @return      ZStatus_t
 *
 **************************************************************************************************/
static ZStatus_t sendDevASEGenResp( DevSendASEGenResp *pASEGenResp )
{
  bool disableDefaultRsp;
  uint8 addrModeMask = ADDR_MODE_UNICAST + ADDR_MODE_GROUPCAST + ADDR_MODE_BROADCAST; // valid addr mode(s) 
  afAddrType_t dstAddr;
  
  uiPrintf( "Sending ASE Generic Response\n" );
  
  if ( gwConvertAddrPbToAfReq( pASEGenResp->dstaddress, &dstAddr, &disableDefaultRsp, addrModeMask ) )
  {
	  disableDefaultRsp = TRUE;
	 return zcl_SendCommand(GW_EP, &dstAddr, ZCL_CLUSTER_ID_HA_ASE_CUSTOM, COMMAND_ASE_CUSTOM_RESP, TRUE, ZCL_FRAME_CLIENT_SERVER_DIR, disableDefaultRsp, 0, zcl_TransID, pASEGenResp->apppkt.len, pASEGenResp->apppkt.data);
  }
