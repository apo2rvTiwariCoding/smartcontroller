...
signed char system_send_reset_request(bool maintain_network_information)
{
	pkt_buf_t * pkt = NULL;
	uint8_t len = 0;
	...
	if (pkt)
	{
		...
		{
			printf("system_send_reset_request: Could not send msg\r\n");
	...
	{
		printf("system_send_reset_request:Could not pack msg\r\n");
