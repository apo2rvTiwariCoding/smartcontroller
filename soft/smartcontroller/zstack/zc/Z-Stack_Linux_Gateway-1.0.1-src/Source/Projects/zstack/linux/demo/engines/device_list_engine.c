...
void device_process_list_response(pkt_buf_t * pkt, void * cbarg)
{
	...
	if (msg)
	{
...
#ifdef ASE
			InformEventToApp(CB_FLAG_DEVICE_LIST, msg, NULL);	
#endif
			ui_refresh_display();
		}
		else
		{
#ifdef ASE
			printf("device_process_list_response: Error: Status FAILURE\r\n");
			InformEventToApp(CB_FLAG_DEVICE_LIST, NULL, NULL);
#endif
	...
	{
#ifdef ASE
		printf("device_process_list_response: Error Could not unpack msg\r\n");
		InformEventToApp(CB_FLAG_DEVICE_LIST, NULL, NULL);
#endif
	...
	if (msg)
	{
		...
		if (found)
		{
			...
			else
			{
				...
				{
                                        displayDeviceInfo(&ds_device_table[i]);
...
#ifdef ASE 
			InformEventToApp(CB_FLAG_DEVICE_INDICATION, msg->deviceinfo, NULL);
#endif
...
signed char device_send_list_request(void)
{
	pkt_buf_t * pkt = NULL;
	uint8_t len = 0;
	...
	//printf("device_send_list_request: Sending NWK_GET_DEVICE_LIST_REQ\r\n");
	...
	if (pkt)
	{
		...
		if (si_send_packet(pkt,	(confirmation_processing_cb_t)&device_process_list_response, NULL) != 0)
		{
			...
			//printf("device_send_list_request: Error: Could not send msg\r\n");
	...
	else
	{
		...
		//printf("device_send_list_request: Error: Could not pack msg\r\n");
