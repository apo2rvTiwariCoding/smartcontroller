...
void nwk_process_ready_ind(pkt_buf_t * pkt)
{
	...
	printf("nwk_process_ready_ind: Received NWK_ZIGBEE_NWK_READY_IND");
	...
	else
	{
		...
		printf("nwk_process_ready_ind: Error: Could not unpack msg");
	}
}
...
void nwk_process_info_cnf(pkt_buf_t * pkt, void * cbarg)
{
	...
	printf("nwk_process_info_cnf: Received NWK_ZIGBEE_NWK_INFO_CNF\r\n");

	msg = nwk_zigbee_nwk_info_cnf__unpack(NULL, pkt->header.len, pkt->packed_protobuf_packet);

	if (msg)
	{
		//UI_PRINT_LOG("MAVENDEBUG, !!!! msg->status = %d !!!!!", msg->status);
		...
		if (msg->status == NWK_NETWORK_STATUS_T__NWK_UP)
		{
			...
			printf("nwk_process_info_cnf: Network ready\r\n");
		}
		else
		{
			ds_network_status.state = ZIGBEE_NETWORK_STATE_INITIALIZING;
			printf("nwk_process_info_cnf: Network not ready, INIT\r\n");
		}
	...
	else
	{
		...
		printf("nwk_process_info_cnf: Error: Could not unpack msg\r\n");
	}
}
