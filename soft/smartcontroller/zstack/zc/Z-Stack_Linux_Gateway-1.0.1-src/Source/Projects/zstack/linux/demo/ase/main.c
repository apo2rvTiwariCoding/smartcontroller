...
void NewDeviceInd(device_info_t *DeviceEntry)
{
	...
	printf("zone_occu = %d\nunoccu_rep = %d\n",asemprofile->zone_occu, asemprofile->unoccu_rep);//printing asemp profile
...
//		printf("Response: %s\n", CmdBuffer);
		SendGenRespPkt(DeviceEntry->ieee_addr, strlen(CmdBuffer), (unsigned char *)CmdBuffer);
		printf("General Response Send to %016llx\n", DeviceEntry->ieee_addr);
		resp = false;
	}
...
static void SendGenReqResp(unsigned char IsReqFlag)
{
	...
	else if(IsReqFlag == SEND_GEN_RESP_PKT)
	{
		...
		{
			printf("Send generic response successful..\r\n");
		}
	}
...
int ASEDataReceived(unsigned short NwkAddr, unsigned short EndPoint, unsigned short DeviceId, unsigned short ClusterId, unsigned char IsReqFlag, char *Message, unsigned char MessageLen, unsigned long long IEEEAddr)
{
	if(IsReqFlag == SEND_GEN_REQ_PKT)
	{
		...
		//UnsolicitedSensorInfoRequest *unSoliRequest =(UnsolicitedSensorInfoRequest *)Message;
		//printf("Avg Temp: %d\r\n",(int) unSoliRequest->avg_temp);
		ASEMPMessage *ASEMPmsg = (ASEMPMessage *)Message;
		if (ASEMPmsg->protocolID == 0X02)
		{
			printf("ASEMPmsg->protocolID: %d\r\n", (int)ASEMPmsg->protocolID);
			UnsolicitedSensorInfoRequest *unSoliRequest =(UnsolicitedSensorInfoRequest *)Message;
			printf("Avg Temp: %d\r\n",(int) unSoliRequest->avg_temp);
		}
		printf("========================================\r\n");
...
{
	printf("\r\n============================= MENU [VER: %d]=================================\r\n", SAMPLE_APP_VERSION);
	printf("On new device indication a general response packet will be sent.\r\n1. Form network\r\n2. Allow device join\r\n3. Get device list\r\n4. Remove device\r\n5. Send on req\r\n6. Send off req\r\n7. Send generic req			\r\n8. Send generic resp\r\n9. Get network state\r\n10. Read smart meter\r\n11. Reset network\r\n12. Read basic cluster\r\n13.Enable Response\r\n0. Exit application\r\n");
	printf("=====================================================================\r\n");
}
		...
		switch(Choice)
		{
			...
			case 12:
				...
				break;
                        ...
                        /* Read attributes of basic cluster. */
                        case 13:
				printf("Response before: %d\r\n", resp);
				resp=true;
				printf("Response:after %d\r\n", resp);
                                break;
