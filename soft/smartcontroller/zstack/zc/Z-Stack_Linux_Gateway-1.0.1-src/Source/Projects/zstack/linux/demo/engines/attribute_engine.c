...
void attr_process_read_attribute_response(pkt_buf_t * pkt)
{
	...
	if (msg)
	{
		...
		if (msg->status == GW_STATUS_T__STATUS_SUCCESS)
		{
			...
			printf("attr_process_read_attribute_response: Status SUCCESS.");
			...
			InformEventToApp(CB_FLAG_ATTRIB_READ_RESP, attr_recvd, NULL);
		}
		else
		{
			//UI_PRINT_LOG("attr_process_read_attribute_response: Error: Status FAILURE.");
			printf("attr_process_read_attribute_response: Error: Status FAILURE.");

			/*  Inform to ASE's custom library */
			InformEventToApp(CB_FLAG_ATTRIB_READ_RESP, NULL, NULL);
		}
	...
	else
	{
		...
		printf("attr_process_read_attribute_response: Error: Could not unpack msg.");
...
/* ASE Change: */
#ifdef ASE
uint8_t GetEndPointFromDevTable(uint64_t IEEEAddr)
{
	int Iterator = 0;

//	printf("ATTR ENGN, IEEE ADDR, %016llx\r\n", IEEEAddr);
	...
	for (Iterator = 0; Iterator < MAX_DEVICE_LIST; Iterator++)
	{
		...
		if ((ds_device_table[Iterator].valid) && (ds_device_table[Iterator].device_status != GW_DEVICE_STATUS_T__DEVICE_REMOVED) && (ds_device_table[Iterator].ieee_addr == IEEEAddr))
		{
			printf("ATTR ENGN, IEEE ADDR, %016llx, ENDPT, %u\r\n", IEEEAddr,ds_device_table[Iterator].ep_list[ds_device_table[Iterator].selected_endpoint_index].endpoint_id);	
			/* Get endpoint of respective IEEE address. */
			return ds_device_table[Iterator].ep_list[ds_device_table[Iterator].selected_endpoint_index].endpoint_id;	

		}
	}

//	printf("ERROR:ATTR ENGN, ENDPT NT FND, IEEE ADDR, %llu\r\n", IEEEAddr);
