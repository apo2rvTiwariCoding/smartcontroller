...
{
	PrintNameOfProcess();
	  printf(", %s, %d, %s, ANANDV, HERE, %x \r\n", __func__, __LINE__, __FILE__, pkt->clusterId);
  zcl_TaskID = task_id;
}
#endif
...
zclProcMsgStatus_t zcl_ProcessMessageMSG( afIncomingMSGPacket_t *pkt )
{
  ...
  if(pInPlugin != NULL)
 	 printf("PLUGIN, START, %x, END, %x\r\n", pInPlugin->startClusterID, pInPlugin->endClusterID);
  else
	  printf("ERROR TO FIND PLIUGON\r\n");
...
static zclLibPlugin_t *zclFindPlugin( uint16 clusterID, uint16 profileID )
{
  ...
  while(pLoop != NULL)
  {
   printf("PLUGIN, START, %x, END, %x, %x\r\n", pLoop->startClusterID, pLoop->endClusterID, profileID);
    pLoop = pLoop->next;
  }
  pLoop = plugins;
  while ( pLoop != NULL )
 ...
 */
uint8 zclGetClusterOption( uint8 endpoint, uint16 clusterID )
{
  uint8 option;
...
/*********************************************************************
 * @fn      zclUpdateClusterOption
 *
 * @brief   Update existing security options, or register new ones if not previously exist
 *
 * @param   endpoint - Application's endpoint
 * @param   clusterID - cluster ID looking for
 * @param   optionMask - only bits set to 1 here will be updated
 * @param   optionValue - bits which are enabled by optionMask will be overwritten by the correspondin bits from this argument
 *
 * @return  zSuccess / zFailure
 */
ZStatus_t zclUpdateClusterOption( uint8 endpoint, uint16 clusterID, uint8 optionMask, uint8 optionValue )
{
  zclOptionRec_t *pOption;
  ...
  if ( pOption != NULL )
  {
    pOption->option &= ~optionMask;
    pOption->option |= optionValue;
  }
  else
  {
    pOption = zcl_mem_alloc(sizeof(zclOptionRec_t));
    if (pOption == NULL)
    {
      return ZFailure;
    }
    
    pOption->clusterID = clusterID;
    pOption->option = optionValue;
    if (zcl_registerClusterOptionList( endpoint, 1, pOption ) != ZSuccess)
    {
      zcl_mem_free(pOption);
