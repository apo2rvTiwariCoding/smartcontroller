...
#define MAX_NEIGHBOR_CNT		30		/*!< Maximum neighbor count for each node in the network. */
#define MAX_NODE_CNT			60		/*!< Maximum node that will be present in the network. */

/******************************************************************************
 * Types
 *****************************************************************************/
/*! \brief Stores status of network address resolution response. */
typedef enum 
{
  NWK_ADDR_RESOLUTION_STATUS_SUCCESS = 0,
  NWK_ADDR_RESOLUTION_STATUS_INVALID_REQTYPE = 128,
  NWK_ADDR_RESOLUTION_STATUS_DEVICE_NOT_FOUND = 129,
  NWK_ADDR_RESOLUTION_STATUS_INVALID_EP = 130,
  NWK_ADDR_RESOLUTION_STATUS_NOT_ACTIVE = 131,
  NWK_ADDR_RESOLUTION_STATUS_NOT_SUPPORTED = 132,
  NWK_ADDR_RESOLUTION_STATUS_TIMEOUT = 133,
  NWK_ADDR_RESOLUTION_STATUS_NO_MATCH = 134,
  NWK_ADDR_RESOLUTION_STATUS_NO_ENTRY = 136,
  NWK_ADDR_RESOLUTION_STATUS_NO_DESCRIPTOR = 137,
  NWK_ADDR_RESOLUTION_STATUS_INSUFFICIENT_SPACE = 138,
  NWK_ADDR_RESOLUTION_STATUS_NOT_PERMITTED = 139,
  NWK_ADDR_RESOLUTION_STATUS_TABLE_FULL = 140,
  NWK_ADDR_RESOLUTION_STATUS_NOT_AUTHORIZED = 141,
  NWK_ADDR_RESOLUTION_STATUS_BINDING_TABLE_FULL = 142
} NWK_ADDR_RESOLUTION_STATUS;

/*! \brief Stores information of the neighbor like IEEE address, network address, LQI, depth, relation etc. */
typedef struct
{
  ...
  unsigned char permitjoining;
  uint32_t depth;
  uint32_t lqi;
}NEIGHBOR_INFO;

/*! \brief Stores neighbor count and information of neighbor. */
typedef struct 
{
	unsigned char NeighborCnt;
	unsigned char StrgeIndx;
	NEIGHBOR_INFO NeighborInfo[MAX_NEIGHBOR_CNT];

}TOPOLOGY_INFO;

/*! \brief Stores network address resolution response details. */
typedef struct
{
  uint8_t status;
  uint64_t ieeeaddr;
